...
public abstract class MineshaftLanternMixin extends StructurePiece {
    ...
    @Unique
    private static final BlockState lantern = ModRegistry.COPPER_LANTERN.get().getDefaultState ().with(LightableLanternBlock.HANGING, true);
    @Unique
    private static final BlockState torch = Blocks.WALL_TORCH.getDefaultState ();
    ...
    /*
    @Overwrite
    private void placeSupport(WorldGenWorld reader, BoundingBox boundingBox, int minX, int minY, int z, int y, int maxX, Random random) {
        if (this.isSupportingBox(reader, boundingBox, minX, maxX, y, z)) {
            ...
            this.generateBox(reader, boundingBox, minX, minY, z, minX, y - 1, z, fence.with(FenceBlock.WEST, true), CAVE_AIR, false);
            this.generateBox(reader, boundingBox, maxX, minY, z, maxX, y - 1, z, fence.with(FenceBlock.EAST, true), CAVE_AIR, false);
            if (random.nextInt(4) == 0) {
                ...
                if (RegistryConfigs.reg.HAS_MINESHAFT_LANTERN && 0.3 > random.nextFloat()) {
                    ...
                    this.maybeGenerateBlock(reader, boundingBox, random, 0.06F, minX + 1, y, z - 1, lantern.with(WallLanternBlock.FACING, Direction.SOUTH)
                            .with(FaceAttachedHorizontalDirectionalBlock.FACE, AttachFace.WALL).with(LightableLanternBlock.LIT, on));
                    //if(!this.getBlock(reader, minX+1, y + 1, z-1, boundingBox).isAir())
                    this.maybeGenerateBlock(reader, boundingBox, random, 0.06F, minX + 1, y, z + 1, lantern.with(WallLanternBlock.FACING, Direction.NORTH)
                            .with(FaceAttachedHorizontalDirectionalBlock.FACE, AttachFace.WALL).with(LightableLanternBlock.LIT, on));


                } else {
                    this.maybeGenerateBlock(reader, boundingBox, random, 0.05F, minX + 1, y, z - 1, torch.with(WallTorchBlock.FACING, Direction.SOUTH));
                    this.maybeGenerateBlock(reader, boundingBox, random, 0.05F, minX + 1, y, z + 1, torch.with(WallTorchBlock.FACING, Direction.NORTH));
                }
            }
