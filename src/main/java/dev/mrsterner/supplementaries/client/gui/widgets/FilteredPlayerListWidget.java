...
public class FilteredPlayerListWidget implements Widget, NarratableEntry, GuiEventListener {
    ...
    public boolean mouseDragged(double dx, double dy, int key, double mouseX, double mouseY) {
        if (this.isDragging) {
            ...
            f = f * (float) j + 0.5F;
            this.scrollOff = MathHelper.clamp((int) f, 0, j);
            return true;
        }
        return false;
    }
    ...
    public boolean keyPressed(int key, int a, int b) {
        ...
        // down arrow, enter
            ...
            this.scrollOff = MathHelper.clamp(this.scrollOff + 1, 0, this.filtered.size() - ENTRY_PER_SCREEN);
            return true;
        }
        return false;
    }
    ...
    protected final SimplePlayerEntry getEntryAtPosition(double mouseX, double mouseY) {
        ...
        int rel = MathHelper.floor(mouseY - (double) this.y);
