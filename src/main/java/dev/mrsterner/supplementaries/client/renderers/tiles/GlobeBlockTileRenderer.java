...
public class GlobeBlockTileRenderer implements BlockEntityRenderer<GlobeBlockTile> {
    ...
    public void render(GlobeBlockTile tile, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int combinedLightIn,
        ...
        this.renderGlobe(tile.renderData, matrixStackIn, bufferIn, combinedLightIn, combinedOverlayIn, tile.isSepia(), tile.getWorld());

        matrixStackIn.popPose();
    }

    public void renderGlobe(Pair<GlobeBlockTile.GlobeModel, ResourceLocation> data, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay, boolean isSepia, World level) {
        if (data == null) return;
        poseStack.pushPose();
