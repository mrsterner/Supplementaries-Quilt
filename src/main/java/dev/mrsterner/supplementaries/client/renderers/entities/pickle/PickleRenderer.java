...
public class PickleRenderer extends LivingEntityRenderer<AbstractClientPlayer, PickleModel<AbstractClientPlayer>> {
    ...
    private void setModelProperties(AbstractClientPlayer player) {
        ...
        if (this.wasCrouching != c && c) this.axisFacing = -player.getDirection().asRotation();
        this.wasCrouching = c;

        //playermodel.crouching = player.isCrouching();

        HumanoidModel.ArmPose poseRightArm = getArmPose(player, Hand.MAIN_HAND);
        HumanoidModel.ArmPose poseLeftArm = getArmPose(player, Hand.OFF_HAND);
    ...
    protected static HumanoidModel.ArmPose getArmPose(AbstractClientPlayer player, Hand hand) {
    ...
    protected void setupRotations(AbstractClientPlayer player, PoseStack matrixStack, float p_225621_3_, float p_225621_4_, float partialTicks) {
        ...
        if (player.isFallFlying()) {
            ...
            float inclination = MathHelper.clamp(f1 * f1 / 100.0F, 0.0F, 1.0F);
