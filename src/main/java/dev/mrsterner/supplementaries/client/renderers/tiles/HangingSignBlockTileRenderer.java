...
public class HangingSignBlockTileRenderer implements BlockEntityRenderer<HangingSignBlockTile> {
    ...
    public void render(HangingSignBlockTile tile, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn,
        ...
        BlockProperties.SignAttachment attachment = state.get(BlockProperties.SIGN_ATTACHMENT);
        //rotate towards direction
        double dy = attachment == BlockProperties.SignAttachment.CEILING ? 1 : 0.875;

        poseStack.translate(0.5, dy, 0.5);
        if(state.get(HangingSignBlock.AXIS) == Direction.Axis.X) poseStack.mulPose(RotHlpr.Y90);
        ...
        if (lod.isMedium()) {
            ...
            if (!tile.isEmpty()) {
                ...
                if (item instanceof ComplexItem) {
                    MapItemSavedData mapData = MapItem.getSavedData(stack, tile.getWorld());
                    if (mapData != null) {
                        for (int v = 0; v < 2; v++) {
                            poseStack.pushPose();
                ...
                //render item
                    ...
                    BakedModel model = itemRenderer.getModel(stack, tile.getWorld(), null, 0);
                    for (int v = 0; v < 2; v++) {
                        poseStack.pushPose();
