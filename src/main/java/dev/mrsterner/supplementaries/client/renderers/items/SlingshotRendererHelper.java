...
public class SlingshotRendererHelper {
    ...
    public static ItemStack getAmmoForPreview(ItemStack cannon, @Nullable World world, Player player) {
        if (world != null) {
    ...
    public static void grabNewLookPos(Player player) {
        ...
        if (raytrace.getType() == HitResult.Type.BLOCK && start.distanceToSqr(raytrace.getLocation()) > MathHelper.square(Minecraft.getInstance().gameMode.getPickRange())) {
    ...
    public static void renderBlockOutline(PoseStack matrixStack, Camera camera, Minecraft mc) {
        if (LOOK_POS != null) {
            ...
            if (!blockstate.isAir() && world.getWorldBorder().isWithinBounds(pos)) {
                ...
                renderVoxelShape(matrixStack, builder, blockstate.getShape(world, pos,  ShapeContext .of(camera.getEntity())),
