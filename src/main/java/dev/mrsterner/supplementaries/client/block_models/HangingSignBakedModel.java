...
public class HangingSignBakedModel implements IDynamicBakedModel {
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @Nonnull Random rand, @Nonnull IModelData extraData) {
        ...
        if (state != null && state.getBlock() instanceof HangingSignBlock hs) {
            //support & connections
            try {
                var a = state.get(HangingSignBlock.ATTACHMENT);
            ...
            try {
                ...
                if (!fancy) {
                    ...
                    boolean flipped = state.get(HangingSignBlock.AXIS) == Direction.Axis.X;
                    boolean ceiling = state.get(HangingSignBlock.ATTACHMENT) == BlockProperties.SignAttachment.CEILING;
                    if (flipped || ceiling) {
