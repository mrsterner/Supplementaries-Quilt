...
public class ParticleUtil {
    ...
    public static void spawnParticleOnBlockShape(World level, BlockPos pos, ParticleOptions particleOptions,
    ...
    public static void spawnParticleOnBoundingBox(AABB bb, World level, BlockPos pos, ParticleOptions particleOptions,
                                                  UniformInt uniformInt, float maxSpeed) {
    ...
    public static void spawnParticlesOnBlockFaces(World level, BlockPos pos, ParticleOptions particleOptions,
    ...
    public static void spawnParticleOnFace(World level, BlockPos pos, Direction direction, ParticleOptions particleOptions,
                                           float minSpeed, float maxSpeed, boolean perpendicular) {
        Vec3 vec3 = Vec3.atCenterOf(pos);
        int i = direction.getStepX();
        int j = direction.getStepY();
        int k = direction.getStepZ();
        double d0 = vec3.x + (i == 0 ? MathHelper.nextDouble(level.random, -0.5D, 0.5D) : (double) i * 0.6D);
        double d1 = vec3.y + (j == 0 ? MathHelper.nextDouble(level.random, -0.5D, 0.5D) : (double) j * 0.6D);
        double d2 = vec3.z + (k == 0 ? MathHelper.nextDouble(level.random, -0.5D, 0.5D) : (double) k * 0.6D);
        double dx;
        double dy;
        double dz;
        if (perpendicular) {
            dx = i * MathHelper.randomBetween(level.random, minSpeed, maxSpeed);
            dy = j * MathHelper.randomBetween(level.random, minSpeed, maxSpeed);
            dz = k * MathHelper.randomBetween(level.random, minSpeed, maxSpeed);
        } else {
