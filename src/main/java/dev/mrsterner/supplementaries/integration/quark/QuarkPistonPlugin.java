...
public class QuarkPistonPlugin {
    ...
    //not really a plugin
    @Nullable
    public static BlockEntity getMovingTile(BlockPos pos, World world) {
        return PistonsMoveTileEntitiesModule.getMovement(world, pos);
    }

    public static boolean updateMovingTIle(BlockPos pos, World world, BlockEntity tile) {
        ...
        try {
            ...
            if (o instanceof WeakHashMap) {
                WeakHashMap<Level, Map<BlockPos, NbtCompound>> movements = (WeakHashMap<Level, Map<BlockPos, NbtCompound>>) o;
