    ...
    public void tick() {
        ...
        if (blockState.isAir()) {
            ...
            if (!this.level.isClient()) {
                ...
                //fall
                if (!this.onGround) {
                    if (!this.level.isClient() && (this.time > 100 && (pos.getY() <= this.level.getMinBuildHeight() || pos.getY() > this.level.getMaxBuildHeight()) || this.time > 600)) {
                    ...
                    if (!onState.is(Blocks.MOVING_PISTON)) {
                        ...
                        boolean canReplace = onState.canReplace(new DirectionalPlaceContext(this.level, pos, Direction.DOWN,
                        ...
                        if (canReplace && canSurvive) {
                            ...
                            if (this.level.setBlockState(pos, blockState, 3)) {
                                ...
                                if (remaining != 0) {
                                    ...
                                    if (level.getBlockState(above).getMaterial().isReplaceable()) {
                                        if (!this.level.setBlockState(above, blockState, 3)) {
                                            ((ServerLevel) this.level).getChunkSource().chunkMap.broadcast(this,
