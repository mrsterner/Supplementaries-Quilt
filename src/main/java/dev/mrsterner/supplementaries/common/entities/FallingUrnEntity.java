...
public class FallingUrnEntity extends ImprovedFallingBlockEntity {
    ...
    public FallingUrnEntity(EntityType<FallingUrnEntity> type, World level) {
        super(type, level);
    }

    public FallingUrnEntity(World level, BlockPos pos, BlockState blockState) {
        super(ModRegistry.FALLING_URN.get(),level, pos, blockState, false);
        this.setHurtsEntities(1f, 20);
    }

    public static FallingUrnEntity fall(World level, BlockPos pos, BlockState state) {
    ...
    public boolean causeFallDamage(float height, float amount, DamageSource source) {
        ...
        if (this.getDeltaMovement().lengthSqr() > 0.5*0.5) {
            ...
            if (!this.isSilent()) {
                level.syncWorldEvent(1045, this.blockPosition(), 0);
            }
        }
        return r;
    }
    ...
    private void shatter() {
        ...
        NbtCompound tag = this.blockData;
