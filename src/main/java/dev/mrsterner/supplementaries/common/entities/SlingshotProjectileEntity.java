...
public class SlingshotProjectileEntity extends ImprovedProjectileEntity implements IEntityAdditionalSpawnData {
    ...
    public Lazy<Integer> light = Lazy.of(() -> {
        ...
        if (item instanceof BlockItem) {
            ...
            return b.getLightEmission(b.getDefaultState (), this.level, this.blockPosition());
        }
        return 0;
    });
    ...
    public SlingshotProjectileEntity(PlayMessages.SpawnEntity spawnEntity, World world) {
            ...
            if (item instanceof BlockItem bi) {
                ...
                if (block.builtInRegistryHolder().is(BlockTags.ENDERMAN_HOLDABLE) || ServerConfigs.cached.UNRESTRICTED_SLINGSHOT) {
                    if (enderman.getCarriedBlock() == null) {
                        enderman.setCarriedBlock(block.getDefaultState ());
    ...
    protected void onHitBlock(BlockHitResult hit) {
        ...
        if (owner instanceof Player player && player.getAbilities().mayBuild ) {
            ...
            PlayerInteractEvent.RightClickBlock blockPlaceEvent = new PlayerInteractEvent.RightClickBlock(player, Hand.MAIN_HAND, hit.getBlockPos(), hit);
            ...
            if (!success) {
                ...
                success = ItemsUtil.place(item,
                        new ItemPlacementContext(this.level, p, Hand.MAIN_HAND, this.getItem(), hit)).consumesAction();
        ...
        if (!this.isNoPhysics()) {
            ...
            if (this.tickCount > 1 && d * this.tickCount > 1.5) {
                if (this.isNoGravity()) {
                    ...
                    Vec3 offset = MathHelperUtils.changeBasisN(movement, rot);
