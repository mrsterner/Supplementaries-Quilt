...
public class FluteItem extends InstrumentItem implements IThirdPersonAnimationProvider,
    ...
    /*
    @Override
    public ActionResult interactLivingEntity(ItemStack stack, Player playerIn, LivingEntity target, Hand hand) {
        if (interactWithPet(stack, playerIn, target, hand)) {
            return ActionResult.success(playerIn.level.isClient());
        }
    ...
    public static boolean interactWithPet(ItemStack stack, Player player, Entity target, Hand hand) {
        if (!(target instanceof LivingEntity)) return false;
        NbtCompound c = stack.getTagElement("Pet");
        if (c == null && (
    ...
    public ActionResultHolder<ItemStack> use(World worldIn, Player playerIn, Hand handIn) {
        ...
        if (!worldIn.isClient()) {
            ...
            NbtCompound com = stack.getTagElement("Pet");
            if (com != null) {
        ...
        return ActionResultHolder.consume(stack);
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable World worldIn, List<Component> tooltip, TooltipFlag flagIn) {
        ...
        NbtCompound tag = stack.getTagElement("Pet");
        if (tag != null) {
    ...
    public void spawnNoteParticle(World level, LivingEntity entity, int note) {
        ...
        float yRot = -MathHelper.wrapDegrees(entity.yHeadRot) * toRad;
        ...
        int mirror = entity.getMainArm() == HumanoidArm.RIGHT ^ entity.getUsedItemHand() == Hand.MAIN_HAND ? -1 : 1;
    ...
    public void animateItemFirstPerson(LivingEntity entity, ItemStack stack, Hand hand, PoseStack matrixStack, float partialTicks, float pitch, float attackAnim, float handHeight) {
        ...
        if (entity.isUsingItem() && entity.getUseItemRemainingTicks() > 0 && entity.getUsedItemHand() == hand) {
            //bow anim
            int mirror = entity.getMainArm() == HumanoidArm.RIGHT ^ hand == Hand.MAIN_HAND ? -1 : 1;
            ...
            float sin = MathHelper.sin((timeLeft - 0.1F) * 1.3F);
    ...
    private <T extends LivingEntity> void animateHands(HumanoidModel<T> model, T entity, boolean leftHand) {
        ...
        float downFacingRot = MathHelper.clamp(headXRot, 0f, 0.8f);
        ...
        offHand.yRot = (float) MathHelper.clamp((RotHlpr.wrapRad(mainHand.yRot) - 1 * mirror) * 0.2, -0.15, 0.15) + 1.1f * mirror;
        offHand.xRot = RotHlpr.wrapRad(mainHand.xRot - 0.06f);


        //shoulder joint hackery
        float offset = leftHand ? -MathHelper.clamp(headYRot, -1, 0) :
                MathHelper.clamp(headYRot, 0, 1);
    ...
    public static float getMaxHeadXRot(float xRot) {
        return MathHelper.clamp(xRot, (-(float) Math.PI / 2.5F), ((float) Math.PI / 2F));
    }
