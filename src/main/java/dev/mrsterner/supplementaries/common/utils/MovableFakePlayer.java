...
public class MovableFakePlayer extends FakePlayer {
    public MovableFakePlayer(ServerWorld level, GameProfile name) {
        super(level, name);
    }
     ...
     */
    public static MovableFakePlayer get(ServerWorld level, GameProfile username) {
    ...
    public static void unloadLevel(ServerWorld level) {
        FAKE_PLAYERS.entrySet().removeIf(entry -> entry.get().level == level);
    }
}
