...
public class FlowerPotHandler {
    ...
    public static void init() {
        ...
        for (FlowerPotBlock pot : emptyPots) {
            ...
            FULL_POTs_BLOCKSTATES_LIST.addAll((pot.getFullPotsView()).values().stream().map(s -> s.get().getDefaultState ()).collect(Collectors.toList()));
        }
    }
    ...
    private static void registerCompatFlower(String itemRes) {
        Item item = ForgeRegistries.ITEMS.get(new ResourceLocation(itemRes));
