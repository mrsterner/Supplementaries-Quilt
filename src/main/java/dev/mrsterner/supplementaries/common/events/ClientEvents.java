    ...
    public static void onClientTick(TickEvent.ClientTickEvent event) {
        if (event.phase == TickEvent.Phase.END && Minecraft.getInstance().level != null) {
            ...
            if (p != null) {
                ...
                isOnRope = (p.getX() != p.xOld || p.getZ() != p.zOld) && state.is(ModRegistry.ROPE.get()) && !state.get(RopeBlock.UP) &&
    ...
    public static void onCameraSetup(EntityViewRenderEvent.CameraSetup event) {
        ...
        if (p != null && !Minecraft.getInstance().isPaused()) {
            if (isOnRope || wobble != 0) {
                ...
                event.setRoll(event.getRoll() + MathHelper.sin((float) (wobble * 2 * Math.PI)) * ClientConfigs.cached.ROPE_WOBBLE_AMPLITUDE);
            }
        }
    }
