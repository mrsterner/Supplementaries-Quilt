...
public abstract class MimicBlock extends Block implements IForgeBlock {
    ...
    //THIS IS DANGEROUS
    @Override
    public float getDestroyProgress(BlockState state, Player player, BlockView worldIn, BlockPos pos) {
    ...
    public List<ItemStack> getDrops(BlockState state, LootContext.Builder builder) {
        ...
        if (builder.getParameter(LootContextParams.BLOCK_ENTITY) instanceof IBlockHolder tile) {
            ...
            if(builder.getParameter(LootContextParams.THIS_ENTITY) instanceof ServerPlayerEntity player) {
                if (!heldState.canHarvestBlock(builder.getWorld(), new BlockPos(builder.getParameter(LootContextParams.ORIGIN)), player)) {
                    return drops;
                }
            }
